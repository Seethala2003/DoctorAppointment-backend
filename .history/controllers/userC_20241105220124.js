...
const getAllDoctorsController = async (req, res) => { 
  try { const doctors = await Doc.find({}); res.status(200).json({ success: true, data: doctors }); } catch (error) { console.error("Fetching doctors error:", error); res.status(500).json({ message: "Something went wrong", success: false }); } }; module.exports = { registerController, loginController, registerDoctorController, getAllDoctorsController };