...
const getStatusApproveController = async (req, res) => {
  try {
    ...
    if (user) { user.notification.push({ type: 'doctor-account-approved', message: 'Your Doctor account has been approved', onClickPath: '/notification', }); await user.save(); }

    if (status === "approved") {
      user.isDoctor = true; // Update the isDoctor field
    } else {
      user.isDoctor = false;
    }
