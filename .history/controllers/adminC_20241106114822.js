...
const getStatusApproveController = async (req, res) => {
  try {
    ...
    return res.status(201).send({ message: 'Successfully updated approval status of the doctor!', success: true, data: doctor, }); } catch (error) { console.error('Error updating approval status:', error); return res.status(500).send({ message: 'Something went wrong', success: false }); } };
